...
gt = np.load("/workspaces/IsaacLab-Arena/eval_output/rerender/probe_set/depth/episode_000000.npz")["depth"][0].astype(np.float32)
...
for name in ["rerendered", "dataset"]:
    ...
    for label, m in [("whole frame", valid), ("tabletop band", table)]:
        ...
        print(f"  {label:14s} raw={raw*100:6.2f}cm  med(pred/gt)={ratio:5.2f}x"
