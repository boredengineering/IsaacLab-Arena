...
PYTHON_PACKAGES = frozenset({
    "isaaclab_arena", "isaaclab_arena_examples", "isaaclab_arena_environments",
    "isaaclab_arena_g1", "isaaclab_arena_gr00t", "isaaclab_arena_openpi",
})
REGISTRIES = tuple(Path(folder) for folder in (
    "isaaclab_arena/assets", "isaaclab_arena/relations", "isaaclab_arena/tasks",
))
...
FRONTEND_REFERENCES = re.compile(
    r'''(?:from\s*|import\s*\(|import\s*|new URL\(\s*|url\(\s*)['"](\.[^'"]+)'''
)
...
def stage(root, destination, browser, backend_tests=()):
    ...
    if (browser and backend_tests or len(backend_tests) != len(set(backend_tests))
        ...
        if browser:
            ...
            while queue:
                ...
                for reference in FRONTEND_REFERENCES.findall(data.decode("utf-8")):
                    ...
                    candidates = [base, *(Path(str(base) + suffix) for suffix in (".ts", ".tsx", ".js", ".jsx")),
