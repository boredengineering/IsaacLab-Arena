...
Plan 04 P1/G04-13/A04-17 now compares S1 side-effect-free declarations/explicit bootstrap, S2 runtime-faithful isolated initialization, S3 an existing owned-process boundary, and S4 complementary dependency-injected contract tests. S1's metadata proposal is **conditional**, not the required solution; S4 cannot replace real E1 proof. The assistant recommends assessing S2 before a broad S1 refactor, but **the user has selected no solution**. Keep the workflow/DB → real registry/schema integration → native calibration → live pilot evidence levels distinct. Discussion and documentation only; the next step is choosing the architecture/effect boundary, then preparing and approving its bounded contract. NR04-12 records this reframing; final doc evidence belongs to `outputs/workflow/plan04-planning/runtime-boundary-discussion/`.
