...
- [P04-I01: Native integration defects and bounded revalidation](plan04_implementation/01-native-integration-defects.md) — Verified native-only acceptance on a5; all three original and two additional launches remain consumed.
- [P04-I02: Installed visual assessment of retained native evidence](plan04_implementation/02-installed-visual-assessment.md) — Issued; BLOCKED after installed worker release, zero provider sends/result. Worker absent, owner epoch 6 unretired, API still live/stopping with `cleanup_unknown`; operation deadline expired. Authorized private binding is installed; three sends remain unspent, one model-call/190s reservation preserved. No native authority or accounting reset; exact lifecycle-recovery decision required.
- [P04-I02 issue-recovery proposal](plan04_implementation/02-installed-visual-assessment.md#issue-recovery-proposal--draft-for-review) — Saved for further review, **NOT ISSUED**. Lifecycle recovery precedes any conditional successor assessment; cumulative accounting is preserved. After reboot, use the [canonical restart checkpoint](dashboard_cli_workflow_parity/research-stack-implementation-handoff.md#resume-after-a-computer-restart); old process observations are not current PID authority.
