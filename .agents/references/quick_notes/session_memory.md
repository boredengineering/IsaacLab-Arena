...
* **The cheapest remedy is a second viewpoint, and it was never costed.** §21's camera ablation (arXiv:2605.28736) shows range and bearing are carried by *different views* — removing the side camera spikes depth errors to **40–65%**; removing the on-arm camera shifts failure mass into **lateral** error (30–45%) instead. Range from two views with baseline against the approach axis is **triangulation, not inference**, needs **no depth sensor** (so it does not violate the deployment constraint), and dominates every method in §20 on cost. Open question: does the G1 policy consume more than the head camera at inference, and does the embodiment expose a usable second view?
...
* **Plan sync**: all of the above landed in `plans/geometry_supervision_evidence_repair_plan.md` **v2.1** (2026-09-06) — §2.4 third removal + loss-form gap, §2.6 loss recipes, **§2.8** (range/bearing asymmetry + second view), **§2.9** (verified absences), new **W3b** (price a second view) and **W9** (residual), risks 6–7, open decision 5. No v2 finding was retracted.
