...
```mermaid
flowchart TD
    GA["A: VERIFIED native-only a5; allocation exhausted"] --> GB["B: Revised P04-I02 accounting-only prompt; resume not issued"]
    GB --> GC["C: One conditional VLM call; no native work; readback and replay"]
    GC --> VP["Remaining scene validation/calibration gates; separate authority"]
    VP --> GD["D: Full installed scene workflow; explicitly budgeted repair if needed"]
    GD --> GE["E: Later GR00T two-seed policy pilot and full-plan readback"]
```
