...
Saved asset and scene previews are recovered from an authenticated read-only catalogue, independently of job-journal retention. Lookup binds the validated canonical scene hash, camera options, resolution, renderer revision and available asset dependency revisions. Missing/corrupt/expired receipts are misses; the UI never replaces an authoritative miss with journal images. The default registry cannot prove mutable remote-asset freshness: its saved PNGs are explicitly **historical**, not fresh cache hits, and are not reused to skip rendering. A server-supplied `LocalAssetRevisions` provider supports verified complete local dependency bundles; automatic registry-wide revision discovery is not implemented.

Use **Render snapshots** beside Assets for a bounded GPU job. Choose Assets/Scene, a scene camera preset, per-asset overrides, and 512/1024 resolution. Isolated robot and reference-prim captures, structured partial errors, measured timings and precise dimension metadata are supported. Robot thumbnails show the authored USD joint pose rather than a simulated initial configuration. Thumbnail variants are at most 256px; zoom loads the full PNG on demand. **Retry image** only fetches an existing artifact.

Automatic previews default off. Explicit consent permits at most three attempts, with a 1.5-second debounce, 10-second minimum interval, and one request in flight. Only validated changes with a catalogue miss qualify; historical receipts do not trigger work. Navigation/document/session changes or reload reset consent, and ambiguous submissions pause automation. Cancellation uses the existing durable job route. Each renderer request is bounded to 360 seconds to accommodate cold material loading.

Browser checks against an explicitly seeded real receipt use `WORKBENCH_E2E_CACHED_SNAPSHOTS=1` and `tests/e2e/asset-previews.spec.ts`; they do not create GPU jobs. `preview-workflow.spec.ts` separately uses labelled mock protocol fixtures for automatic-mode, cancellation and error-path checks; those fixtures are not rendering evidence.
